...
RB.Analytics.prototype = {
    ...
    send: function(action,category,props,callback) {
        ...
        if (this.googleAnalytics) {
            ...
            // NOTE: Remove these next two lines on Production
            console.log( action + ' event sent to Google Analytics.');
        ...
        if (this.amplitude) {
            ...
            if (typeof callback !== 'undefined') {
                ...
                // NOTE: Remove these next two lines on Production
                console.log( action + ' event sent to Amplitude.');
                callback();

                // NOTE: Uncomment the line below on production so the events will actually send
                // amplitude.getInstance().logEvent(category + "_" + action, props, callback)
            } else {

                // NOTE: Remove this console log on production
                console.log( action + ' event sent to Amplitude.');

                // NOTE: Uncomment the line below on production so the events will actually send
                // amplitude.getInstance().logEvent(category + "_" + action, props);
            }
