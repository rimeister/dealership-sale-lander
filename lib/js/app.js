...
NAMESPACE.Carousel.prototype = {
	...
	init: function() {
		...
		if (!this.initialized) {
			...
			this.carouselInnerEl = this.el.getElementsByClassName('carousel-inner')[0];
	...
	showSlide(index) {
		...
		this.carouselInnerEl = this.el.getElementsByClassName('carousel-inner')[0];
		this.carouselInnerEl.style.marginLeft = '-'+(100*index)+'%';
	...
	nextSlide: function(callback) {
		...
		// If you've reached the end and it's set to loop
		if (this.loop && this.currentSlideIndex == this.numSlides-1) {
	...
	startAutoSlide: function(ms,callback) {
		...
		if (!this.timer) {
			...
			this.__nextSlideTimerFn.callback = callback;
			// Set an interval
			this.timer = setInterval(this.__nextSlideTimerFn.bind(this),ms);
		} else {
	...
	// PRIVATE METHODS
	__nextSlideTimerFn: function(){
		this.nextSlide(this.__nextSlideTimerFn.callback);	
	},
